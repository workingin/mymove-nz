...
(function (root, factory) {
    ...
    MediaBox.prototype = {
        ...
        parseUrl: function (url) {
            ...
            } else if (matches = url.match(/https?:\/\/(?:www\.)?vimeo.com\/(?:channels\/|groups\/([^\/]*)\/videos\/|album\/(\d+)\/video\/|)(\d+)(?:\/([0-9a-zA-Z]+))?(?:$|\/|\?)/)) {
                service.provider = "vimeo";
                service.id       = matches[3];
                service.hash     = matches[4];
            } else {
        ...
        render: function (service) {
            ...
            if (service.provider === 'youtube') {
                ...
                // Unlisted and domain-restricted videos only play if the link's hash is carried into the player URL.
                if (service.hash) {
                    params.h = service.hash;
                }
            } else {
                throw new Error("Invalid video URL");
            }

            urlParams = this.serialize(params);
