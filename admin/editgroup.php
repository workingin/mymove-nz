...
if (!empty($_GET['log_id'])) {
    $logid = $_GET['log_id'];
    $groupinfo = $functions->returnGroupInfo($logid);
} else {
    ...
    <form action="includes/adminprocess.php" id="user-groups-edit" class="form-horizontal" method="post">
        ...
        <div class="modal-body" id="modal-body">                                   
            ...
            <div class="form-group">
                ...
                <div class="col-md-8">
                    <select for="add-user" name="add-user[]" class="chosen-select" data-placeholder="Click here.." style="width: 250px;" multiple <?php if ($groupinfo["group_level"] == '1') { echo 'disabled'; } ?>>
                        ...
                        <div class="panel-body">
                            ...
                            while ($row = $result->fetch()) {
                                ...
                                if ($row['username'] != ADMIN_NAME) {
                                    ...
                                    echo "<a href='includes/adminprocess.php?remove=" . $row['id'] . "&group_id=" . $row['group_id'] . "&stop=" . $stop . "&form_submission=remove_groupmember&" . Csrf::queryParam() . "' title='Remove Group Member' class='btn btn-danger confirmation_deleteuser'>";
                                    echo "<i class='fas fa-times'></i></a></div>";
                                }
                                echo "</td></tr>";
                            }
                            ?>
        ...
        <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-primary <?php if ($groupinfo["group_level"] == '1') { echo 'hidden'; } ?>" id="submit" >Edit Group</button>
        </div>
    </form>
</div>
