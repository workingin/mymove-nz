...
class User {	
	...
	public function getUser(){		
		if($this->id) {
			$sqlQuery = "
			SELECT id, first_name, last_name,shortbio,jobtitle, email, type, deleted
			FROM ".$this->userTable." 			
			WHERE id = ? ";
			$stmt = $this->conn->prepare($sqlQuery);
	...
	public function insert(){
		...
		if($this->email && $this->password) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->userTable."(`first_name`, `last_name`, `jobtitle`,  `shortbio`,`email`, `password`, `type`, `deleted`)
				VALUES(?,?,?,?,?,?,?,?)");
			...
			$stmt->bind_param("ssssii", $this->first_name, $this->last_name, $this->jobtitle, $this->shortbio, $this->email, md5($this->password), $this->type, $this->deleted);
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET first_name= ?, last_name = ?,jobtitle = ?, shortbio = ?, email = ?, type = ?, deleted= ?
				WHERE id = ?");
			...
			$stmt->bind_param("ssssssii", $this->first_name, $this->last_name,$this->jobtitle,$this->shortbio, $this->email, $this->type, $this->deleted, $this->id);
