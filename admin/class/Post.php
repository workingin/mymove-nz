...
class Post {	
	...
	public function getPostsListing(){		
		...
		$sqlQuery = "
			SELECT p.id, p.title, p.longdesc,p.category_id, u.first_name, u.last_name, p.status, p.created, p.updated, c.name 
	...
	public function getPost(){		
		if($this->id) {
			$sqlQuery = "
				SELECT p.id, p.title, p.message,p.longdesc,p.filename,p.thumbnail,p.author_id, p.category_id, p.status, p.created, p.updated, c.name 
				FROM ".$this->postTable." p
				LEFT JOIN ".$this->categoryTable." c ON c.id = p.category_id
				WHERE p.id = ? ";
			$stmt = $this->conn->prepare($sqlQuery);
	...
	public function insert(){
		...
		if($this->title && $this->message) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->postTable."(`title`, `message`,`longdesc`, `filename`, `thumbnail`,`author_id`, `category_id`, `userid`, `status`, `created` , `updated`)
				VALUES(?,?,?,?,?,?,?,?,?,?,?)");
			...
			$stmt->bind_param("ssssssiisss", $this->title, $this->message,$this->longdesc,$this->filename,$this->thumbnail,$this->author_id, $this->category, $this->userid, $this->status, $this->created, $this->updated);
		...
		$this->error = 'Title and Short Description are required.';
		return false;
	}
	...
	public function update(){
		...
		if($this->id) {
			$stmt = $this->conn->prepare("
				...
				SET title= ?, message = ?, longdesc = ?, filename = ?,thumbnail = ?, author_id = ?, category_id = ?, status= ?, updated = ?
				WHERE id = ?");
			...
			$this->status = htmlspecialchars(strip_tags($this->status));
			$this->updated = htmlspecialchars(strip_tags($this->updated));
			$stmt->bind_param("ssssssissi", $this->title, $this->message, $this->longdesc, $this->filename,$this->thumbnail,$this->author_id, $this->category, $this->status, $this->updated, $this->id);
